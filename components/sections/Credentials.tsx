        ...
        <div className="flex items-center justify-center gap-3.5 flex-wrap">
          {pills.map((p) => (
            <RevealOnScroll key={p.label}>
              <div className="flex items-center gap-2 px-4 py-[9px] bg-white rounded-full text-[13px] font-semibold text-d60 transition-all duration-200 hover:text-o hover:-translate-y-px cursor-default">
                {p.icon}
                {p.label}
              </div>
            </RevealOnScroll>
          ...
          {unis.map((u) => (
            <span key={u} className="py-[7px] px-3.5 bg-white rounded-full text-[14px] font-bold text-d30">
              {u}
            </span>
          ))}
        </div>
